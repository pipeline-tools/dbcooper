...
hn_stories() %>%
  count(date = DATE_TRUNC(DATE(creation_date), MONTH)) %>%
  ggplot(aes(month, n)) +
  geom_col() +
  labs(y = "# of Hacker News stories")
```

```{r}
dbc_bigquery("stack", "stackoverflow")

stack_posts_questions() %>%
  count(date = DATE_TRUNC(DATE(creation_date), WEEK)) %>%
  ggplot(aes(date, n)) +
  geom_col()
```

```{r}
dbc_bigquery("sec", "sec_quarterly_financials")
